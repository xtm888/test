...
//        dd([$baseProduct, $detailsProduct]);
        ...
        try {
            ...
            if (session('product_type') == 'physical') {
                $detailsProduct::insert([
                    'id' => $baseProduct->id,
                    'countries' => $detailsProduct->countries,
                    'country_from' => $detailsProduct->country_from
                ]);
            }

            if (session('product_type') == 'digital') {

                if ($detailsProduct->content != null) {
                    $autodelivery = 1;
                } else {
                    $autodelivery = 0;
                }
                ...
                $detailsProduct::insert([
                    ...
                    'content' => $detailsProduct->content
                ]);
            }
        ...
        if ($baseProduct->quantity > 0) {
            Product::where('id', $baseProduct->id);
        }
