...
class Cart
{
     ...
     */
    public function addToCart(Product $product, int $quantity, ?Shipping $shipping = null, string $message = null, $type = 'normal', ?string $coin = "xmr")
    {
        // validate shipping
...
//            throw new RequestException('Selected coin payment is not supported by this product!');
        ...
        $newCartItem->id = Str::uuid(); // generate id for address
    ...
    public function total()
    {
        ...
        foreach (session()->get(self::SESSION_NAME) ?? [] as $item) {
