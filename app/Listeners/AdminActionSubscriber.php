...
class AdminActionSubscriber
{
    ...
    public function subscribe($events)
    {
        $events->listen(
            ...
            [AdminActionSubscriber::class, 'onUserChange']
        );

        $events->listen(
            'App\Events\Admin\ProductDeleted',
            [AdminActionSubscriber::class, 'onProductDelete']
        );

        $events->listen(
            'App\Events\Admin\UserGroupChanged',
            [AdminActionSubscriber::class, 'onUserGroupChange']
        );
        $events->listen(
            'App\Events\Admin\UserPermissionsUpdated',
            [AdminActionSubscriber::class, 'onUserPermissionUpdate']
        );
